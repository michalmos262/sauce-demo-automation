...
test.describe('Login', () => {
  ...
  test('TC-L05: Wrong password shows credentials mismatch error', async ({ loginPage }) => {
    await loginPage.usernameInput.fill(USERS[UserType.STANDARD].username);
  ...
  test('TC-L06: Error dismiss X button clears error', async ({ loginPage }) => {
    await loginPage.loginButton.click();
